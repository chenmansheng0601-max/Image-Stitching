...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_indices: list):
        ...
        self.adjust_channels = nn.Conv2d(320, 3, kernel_size=1)  # 添加 1x1 卷积调整通道数
        # self.adjust_channels = nn.Conv2d(128, 3, kernel_size=1)  # 添加 1x1 卷积调整通道数
        self.act = nn.ReLU(inplace=True) 
        self.conv = nn.Conv2d(2048, 320, 1, stride=1)
        # self.conv = nn.Conv2d(2048, 128, 1, stride=1)


    def forward(self, tensor_list: NestedTensor):
        # xs = self.body(tensor_list.tensors)
        tensor_list = self.adjust_channels(tensor_list)
...
class Joiner(nn.Sequential):
    ...
    def forward(self, x: torch.Tensor):
        xs = self[0](x)  # 直接使用 backbone 的输出 (处理普通 Tensor)
        out: List[torch.Tensor] = []
        pos = []
   
        for name, x in xs.items():
            out.append(x)  # 保存输出特征图

            pos.append(self[1](x).to(x.dtype))  # 确保 dtype 一致
